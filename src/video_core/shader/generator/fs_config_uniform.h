...
/**
 * CPU-side mirror of the `fs_config` std140 uniform block consumed by the PICA fragment ubershader
 * (renderer_deko3d/shaders/ubershader_fsh.glsl and its Vulkan copy). Lets a single generic pipeline
 * emulate any PICA fragment state by reading this block at runtime instead of specializing the
 * shader, so a draw whose specialized pipeline is still compiling can render immediately.
 */
...
static_assert(sizeof(FSConfigUniformData) == 240,
              "FSConfigUniformData does not match the fs_config block in the ubershader");

/// Packs a PICA fragment configuration into the fs_config block.
[[nodiscard]] FSConfigUniformData BuildFSConfigUniform(const FSConfig& config);
